...
class SentEmbeddings():
    ...
    def get_tokenized_sent_embeddings(self, text_obj, if_DS=False, if_EA=False):
        ...
        if(self.embeddings_type=="word2vec" and if_DS==False):
            words_embeddings= self.word_embeddor.get_tokenized_words_embeddings([text_obj.tokens])
        elif(self.embeddings_type=="word2vec" and if_DS==True and if_EA==False):
            tokens_segmented = get_sent_segmented(text_obj.tokens)
            word_embeddings= self.word_embeddor.get_tokenized_words_embeddings(tokens_segmented)
            word_embeddings = splice_embeddings(word_embeddings,tokens_segmented)
        elif (self.embeddings_type == "word2vec" and if_DS == True and if_EA == True):
            tokens_segmented = get_sent_segmented(text_obj.tokens)
            word_embeddings = self.word_embeddor.get_tokenized_words_embeddings(tokens_segmented)
            word_embeddings = context_embeddings_alignment(word_embeddings, tokens_segmented)
            word_embeddings = splice_embeddings(word_embeddings, tokens_segmented)
        else:
            word_embeddings, elmo_mask = self.word_embeddor.get_tokenized_words_embeddings(text_obj.tokens)

        candidate_embeddings_list=[]
        weight_list = get_words_textrank(text_obj.tokens)
        sent_embeddings = get_weighted_average(text_obj.tokens, text_obj.tokens_tagged, weight_list, word_embeddings, embeddings_type=self.embeddings_type)

        for kc in text_obj.keyphrase_candidate:
            start = kc[1][0]
            end = kc[1][1]
            kc_emb = get_candidate_weighted_average(text_obj.tokens, weight_list, word_embeddings[0], start, end,
...
def context_embeddings_alignment(word_embeddings, tokens_segmented):

    """
    Embeddings Alignment
    :param word_embeddings: The embeddings from elmo
...
def splice_embeddings(word_embeddings,tokens_segmented):
    new_word_embeddings = word_embeddings[0:1, :, 0:len(tokens_segmented[0]), :]
    for i in range(1, len(tokens_segmented)):
        emb = word_embeddings[i:i + 1, :, 0:len(tokens_segmented[i]), :]
        new_word_embeddings = torch.cat((new_word_embeddings, emb), 2)
...
def get_weighted_average(tokenized_sents, sents_tokened_tagged, weight_list, embeddings_list, embeddings_type="word2vec"):
...
def get_weight_list(word2weight, tokenized_sents, lamda, database=""):
    weight_list = []
